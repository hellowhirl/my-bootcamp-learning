...
const course = courses.find(function(course) {
  // this is a callback function (or predicate)
...
console.log("arrowCourse", arrowCourse);
...
// Solution 4
while (emptyNumbers.length > 0) emptyNumbers.pop();
...
for (let number of numbers) console.log("for of", number);
...
const allPositive = testArray.every(function(value) {
  // every() - checks if every element in array matches criteria
  return value >= 0;
});
console.log(allPositive);

const atLeastOnePositive = testArray.some(function(value) {
  // some() - if at least one value matches criteria
  return value >= 0;
});
...
// old way:
let sum = 0;
for (let n of purchaseNumbers) sum += n;
console.log(sum);
...
let betterSumOfReduced = purchaseNumbers.reduce(
