...
function except(array, excluded) {
  let finalArray = [];
  for (let n of array) {
    if (!excluded.includes(n)) finalArray.push(n);
  }
  return finalArray;
}
...
let countedNumbers = fifthNumbers.reduce((accumulator, currentValue) => {
  if (currentValue === test) accumulator++;
  return accumulator;
}, 0);
...
function getMax(array) {
  ...
  return array.reduce((a, b) => {
    // reudce method is used here to find single value that is the largest
...
function find(movies) {
  let result = [];
  for (movie of movies) {
    if (movie.year === 2018 && movie.rating > 4) result.push(movie.title);
  }
