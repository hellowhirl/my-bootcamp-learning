...
function createBetterCircle(radius) {
    return { // better to write with return as every time we run this function we'll get a circle object
        radius: radius,
        draw: function(){
            console.log('draw');
        }
...
function createBestCircle(radius) {
    return {
        ...
        draw() { // shorter syntax can be similar to how we define a function outside of an object (just drop the function keyword)
            console.log('draw');
        }
    };
}
...
circle2.draw = function() {console.log('test')};
...
const Triangle1 = new Function('radius', `
this.radius = radius;
this.draw = function() {
    console.log('draw');
}
...
for (let key in trapezoid) 
    console.log(key, trapezoid[key]);

for (let key of Object.keys(trapezoid))
    console.log(key);
...
if ('radius' in trapezoid) console.log('yes');
